...
using System.Text;
using MandarinCreative.Extensions;
...
namespace Twilio.TwiML
{
	...
	public class RequestValidator
	{
		...
		public bool IsValidRequest(HttpContext context, string authToken, string urlOverride)
		{
			...
			if (context.Request.Host.Equals("localhost"))
			{
				return true;
			}
			...
			var fullUrl = string.IsNullOrEmpty(urlOverride) ? context.AbsoluteUrl() : urlOverride;
			...
			if (context.Request.Method == "POST")
			{
				...
				var sortedKeys = context.Request.Form.Keys.OrderBy(k => k, StringComparer.Ordinal).ToList();
